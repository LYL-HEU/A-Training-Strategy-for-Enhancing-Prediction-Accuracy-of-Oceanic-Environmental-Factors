...
def read_split_data(path_in: str, train_rate: float = 0.85):
    data = xr.open_dataset('./data/cur.nc')
    uo = data.sel(time=Time)['uo'][::1, :, ::3, ::3]
    vo = data.sel(time=Time)['vo'][::1, :, ::3, ::3]
    uo = uo[:, :, :-1, :-1]
    vo = vo[:, :, :-1, :-1]
    data = xr.open_dataset('./data/SSH.nc')
    z = np.expand_dims(data.sel(time=Time)['zos'][:, ::3, ::3], axis=1)
    z = z[:, :, :-1, :-1]
    data = xr.open_dataset('./data/wind.nc')
    u10 = np.expand_dims(data.sel(time=Time)['u10'], axis=1)
    v10 = np.expand_dims(data.sel(time=Time)['v10'], axis=1)
    u10 = u10[:, :, :-1, :-1]
    v10 = v10[:, :, :-1, :-1]
    in_data = np.concatenate((u10, v10, z), axis=1)
    out_data = np.concatenate((uo, vo), axis=1)
